package com.github.hfutwifi.utils;
/*
...
public class Parser {
    ...
    private static final String pattern_wifi_regex = "(?i)hfutwifi://([A-Za-z0-9_=-]+)";
    private static final String decodedPattern_wifi_regex = "(?i)^((.+):(\\d+?):(.*):(.+):(.*):([^/]+))";
    private static final String decodedPattern_wifi_obfsparam_regex = "(?i)[?&]obfsparam=([A-Za-z0-9_=-]*)";
    private static final String decodedPattern_wifi_remarks_regex = "(?i)[?&]remarks=([A-Za-z0-9_=-]*)";
    private static final String decodedPattern_wifi_protocolparam_regex = "(?i)[?&]protoparam=([A-Za-z0-9_=-]*)";
    private static final String decodedPattern_wifi_groupparam_regex = "(?i)[?&]group=([A-Za-z0-9_=-]*)";
    ...
    public static List<Profile> findAll_wifi(CharSequence data) {
        Pattern pattern_wifi = getPattern(pattern_wifi_regex);
        Pattern decodedPattern_wifi = getPattern(decodedPattern_wifi_regex);
        Pattern decodedPattern_wifi_obfsparam = getPattern(decodedPattern_wifi_obfsparam_regex);
        Pattern decodedPattern_wifi_protocolparam = getPattern(decodedPattern_wifi_protocolparam_regex);
        Pattern decodedPattern_wifi_remarks = getPattern(decodedPattern_wifi_remarks_regex);
        Pattern decodedPattern_wifi_groupparam = getPattern(decodedPattern_wifi_groupparam_regex);
        ...
        Matcher m = pattern_wifi.matcher(input);
        try {
            List<Profile> list = new ArrayList<>();
            while (m.find()) {
                String uri = new String(Base64.decode(m.group(1).replaceAll("=", ""), Base64.URL_SAFE), "UTF-8");
                Matcher ss = decodedPattern_wifi.matcher(uri);
                if (ss.find()) {
                    Profile profile = new Profile();
                    ...
                    param = decodedPattern_wifi_groupparam.matcher(uri);
                    if (param.find()) {
